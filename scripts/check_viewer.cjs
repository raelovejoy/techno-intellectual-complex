...
const ids=['content','search','cluster','type','review','confidence','theme','reset','dataset-status','cy','fit-map','zoom-in','zoom-out','map-detail','map-focus'];
...
(async()=>{
  ...
  elements.theme.listeners.click();assert.equal(document.documentElement.dataset.theme,'dark');
